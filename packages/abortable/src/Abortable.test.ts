...
describe('Abortable.abort', () => {
  it('iterates the thenables and calls the abort or cancel method if present', () => {
    const abortable = Promise.resolve(1);
    abortable.abort = jest.fn();
    const cancellable = Promise.resolve(2);
...
describe('Abortable.all', () => {
  it('will call the onFulfilled handler once all AbortablePromises have been resolved', () => {
    return Abortable.all([1, Promise.resolve(2), new Promise((resolve) => resolve(Promise.resolve(3)))]).then(results => {
...
describe('Abortable.race', () => {
  it('Will call the onFulfilled handler as soon as oneAbortablePromises has been resolved', () => {
    ...
    const testSlowResolvedAbortablePromise = new Promise((resolve) => setTimeout(resolve, 1000, 'two')) as Abortable.Abortable<number>;
  ...
  it('Will call abort on all of the passed abortables if one of the abortables throws', () => {
    const testSlowResolvedAbortablePromise = new Promise((resolve) => setTimeout(resolve, 2000, 'two')) as Abortable.Abortable<number>;
    testSlowResolvedAbortablePromise.abort = jest.fn();

    const testRejectedAbortablePromise = Promise.reject<string>('reason') as Abortable.Abortable<string>;
