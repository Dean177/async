...
describe('Abortable.props', () => {
  it('returns a promise which resolves with a map of the resolved promise', () =>
    Abortable.props({ a: Promise.resolve(1), b: Promise.resolve('two') }).then(resolvedValue => {
  ...
  it('can be aborted and will abort or cancel each of the values', () => {
    ...
    Abortable.props({ a: abortable, b: cancellable }).abort()
