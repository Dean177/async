...
import { AsyncProps, withAsync } from 'with-async';
import './App.css';
...
const bareBonesEnhance = withAsync((props: TestComponentOwnProps): Promise<TestAsyncValue> =>
  apiCall(props.quantity)
);

export const BareBonesComponent: React.ComponentType<TestComponentOwnProps> = bareBonesEnhance(
  ExampleComponent
);
...
const withAsyncCustom = <OP, T>(
  ...
  withAsync(promiseProducer, { shouldReProduce: shouldReRequest })((props: OP & AsyncProps<T>) => {
    ...
    const enhancedProps: OP & AsyncCustom<T> = Object.assign<{}, OP, AsyncCustom<T>>({}, props, {
      async: result
    });
